...
%union {
  ...
  struct {
    ...
    char * i; //array index
    unsigned int label_num; //current label number
  } node;
}
...
statement: int_var ASSIGN expression {stringstream ss; ss << $3.code << "= " << $1.ret << ", "  << $3.ret << "\n"; $$.code = strdup(ss.str().c_str());}
  ...
  | IF bool_exp THEN statements ENDIF {stringstream ss; $$.label_num = curr_if++; ss << $2.code << "?:= if_true" << $$.label_num << ", " << $2.ret << "\n:= endif" << $$.label_num << "\n: if_true" << $$.label_num << "\n" << $4.code << ": endif" << $$.label_num << "\n"; $$.code = strdup(ss.str().c_str());}
  | IF bool_exp THEN statements ELSE statements ENDIF {stringstream ss; $$.label_num = curr_if++; ss << $2.code << "?:= if_true" << $$.label_num << ", " << $2.ret << "\n:= else" << $$.label_num << "\n: if_true" << $$.label_num << "\n" << $4.code << ":= endif" << $$.label_num << "\n: else" << $$.label_num << "\n" << $6.code << ": endif" << $$.label_num << "\n"; $$.code = strdup(ss.str().c_str());}
  | WHILE bool_exp BEGINLOOP {loopScope = true;} statements ENDLOOP {loopScope = false; stringstream ss; $$.label_num = curr_loop++; ss << ": beginloop" << $$.label_num << "\n" << $2.code << "?:= loopbody" << $$.label_num << ", " << $2.ret << "\n:= endloop" << $$.label_num << "\n: loopbody" << $$.label_num << "\n" << $5.code << ":= beginloop" << $$.label_num << "\n: endloop" << $$.label_num << "\n"; $$.code = strdup(ss.str().c_str());}
  | DO BEGINLOOP {loopScope = true;} statements ENDLOOP WHILE bool_exp {loopScope = false; stringstream ss; $$.label_num = curr_loop++; ss << ": beginloop" << $$.label_num << "\n" << $4.code << $7.code << "?:= beginloop" << $$.label_num << ", " << $7.ret << "\n: endloop" << $$.label_num << "\n"; $$.code = strdup(ss.str().c_str());}
...
read_var: int_var {stringstream ss; ss << ".< " << $1.ret << "\n"; $$.code = strdup(ss.str().c_str());}
...
bool_exp: nots expression comp expression {
  ...
  ss << $2.code << $4.code << ". " << $$.ret << "\n" << $3.code << " " << $$.ret << ", " << $2.ret << ", " << $4.ret << "\n";
  if(strlen($1.code) % 2 != 0) {
  ...
  stringstream ss;
  $$.ret = strdup(var_generator());
  ss << $3.code << $5.code << ". " << $$.ret << "\n" << $4.code << " " << $$.ret << ", " << $3.ret << ", " << $5.ret << "\n";
  if(strlen($1.code) % 2 != 0) {
    char* t = strdup(var_generator());
    ss << ". " << t << "\n" << $1.code << " " << t << ", " << $$.ret << "\n";
    $$.ret = strdup(t);
...
nots: NOT nots {stringstream ss; ss << "!" << $2.code; $$.code = strdup(ss.str().c_str());}
  | %empty {$$.code = (char*)"";}
comp: EQ {$$.code = (char*)"==";}
  | NEQ {$$.code = (char*)"!=";}
  | LT {$$.code = (char*)"<";}
